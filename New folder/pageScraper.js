    ...
    async scraper(browser){
		...
		let pagePromise = (link) => new Promise(async(resolve, reject) => {
			...
			await newPage.evaluate(()=>{
				...
				list.querySelector('.left.listing__title > a').innerHTML += '<span><a href="http://localhost:5600/search?brand=' + brand + '&date=' + reg_date + '&mile=' + mile + '&seat=' + seat + '&door=' + door + '&engine=' + engine + '&hp=' + hp + '&type=' + type + '&fuel=' + fuel + '" target="_blank"><button type="button" class="button button--primary facetFilters" title="price">Price</button></a></span>';
			    }
		...
		//for(link in urls){
			...
			// 	await searchPage.waitForSelector('#dsp-lower-search-btn');
			...
			// 	//await searchPage.click('#dsp-lower-search-btn');

			// 	await searchPage.waitForSelector('.price-block');
			// 	let prices = await page.$$eval('.price-block', prices => {
			// 		// Extract the links from the data
